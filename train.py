...
from src.data_setup import (
    get_train_data_transformed,
    get_test_data_transformed,
    get_data_loader,
    train_test_split,
    load_dataset,
)
...
def main():
    ...
    train_and_valid_data = get_train_data_transformed(
    ...
    test_data = get_test_data_transformed(
        "data/test_set/test_set", IMAGE_SIZE, imagenet_mean, imagenet_std
    )
    ...
    train_loader, valid_loader, test_loader = get_data_loader(
        train_data, valid_data, test_data, BATCH_SIZE, True
    )
    ...
    accuracy = torchmetrics.Accuracy(task="multiclass", num_classes=N_CLASSES).to(
        device
    )
    ...
    print("[INFO] Iniciando o processo de avaliação do modelo")
    ...
    optimizer_fine_tune = torch.optim.AdamW(
        params=model.parameters(), lr=learning_rate_fine_tune
    )
    ...
    print("[INFO] Iniciando o processo de avaliação do modelo após o fine tuning")
    model_fine_tuning_acc = (
        evaluate(model, test_loader, accuracy, device).compute().item()
    )
